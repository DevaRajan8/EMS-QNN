...
def create_enhanced_reservoir_circuit(num_data_features, num_recurrent_features, num_qubits=8, depth=3):
    """Creates the quantum circuit for the reservoir."""
...
def create_observables(num_qubits):
    """Creates a set of Pauli observables for measurement."""
    observables = []
...
def get_enhanced_reservoir_states(X, circuit, estimator, observables, memory_decay=0.8):
    """Processes input data through the quantum reservoir to generate feature states."""
...
def evaluate_predictions(y_true, y_pred, target_names):
    """Calculates and returns evaluation metrics (MSE and R2)."""
    results = {}
